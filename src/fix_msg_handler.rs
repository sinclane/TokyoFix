...
impl FixMsgHandler for MyFixMsgHandler {
    ...
    fn on_logon_request(&self, message: String) {
        ...
        self.parse_fix_message(message.as_str(), &mut hmap);

        let heartbeat_interval = hmap.get(fix_tag::HEARTBT_INT.id()).unwrap().parse(Default::default()).unwrap();
